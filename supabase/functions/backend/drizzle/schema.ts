import { pgTable, bigint, timestamp, doublePrecision, text, boolean, uuid, serial, uniqueIndex, integer, index, jsonb, unique, interval } from "npm:drizzle-orm/pg-core"
...
export const outgoingMessages = pgTable("outgoing_messages", {
	...
	broadcastId: bigint("broadcast_id", { mode: "number" }).notNull().references(() => broadcasts.id, { onUpdate: "cascade" } ),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	audienceSegmentId: bigint("audience_segment_id", { mode: "number" }).notNull().references(() => audienceSegments.id, { onUpdate: "cascade" } ),
