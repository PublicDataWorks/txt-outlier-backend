import { pgTable, index, unique, pgEnum, bigint, timestamp, doublePrecision, text, boolean, uuid, serial, uniqueIndex, integer, jsonb, interval } from "npm:drizzle-orm/pg-core"

export const broadcastsSegments = pgTable("broadcasts_segments", {
	id: serial('id').primaryKey().notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	broadcastId: bigint("broadcast_id", { mode: "number" }).notNull().references(() => broadcasts.id, { onUpdate: "cascade" } ),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	segmentId: bigint("segment_id", { mode: "number" }).notNull().references(() => audienceSegments.id, { onUpdate: "cascade" } ),
...
(table) => {
	return {
		broadcastIdSegmentIdIdx: index("broadcasts_segments_broadcast_id_segment_id_idx").on(table.broadcastId, table.segmentId),
		broadcastIdSegmentIdUnique: unique("broadcast_id_segment_id_unique").on(table.broadcastId, table.segmentId),
	}
});

export const broadcastMessagesSentStatus = pgTable("broadcast_messages_sent_status", {
	id: serial('id').primaryKey().notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	...
	missiveMessageId: uuid("missive_message_id").notNull().references(() => twilioMessages.id, { onUpdate: "cascade" } ),
	missiveConversationId: uuid("missive_conversation_id").notNull().references(() => conversations.id, { onUpdate: "cascade" } ),
},
(table) => {
	return {
		broadcastSentMessagesMissiveMessageIdKey: unique("broadcast_sent_messages_missive_message_id_key").on(table.missiveMessageId),
	}
});
...
export const outgoingMessages = pgTable("outgoing_messages", {
	id: serial('id').primaryKey().notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	recipientPhoneNumber: text("recipient_phone_number").notNull().references(() => authors.phoneNumber, { onUpdate: "cascade" } ),
	message: text("message").notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	broadcastId: bigint("broadcast_id", { mode: "number" }).notNull().references(() => broadcasts.id, { onUpdate: "cascade" } ),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
