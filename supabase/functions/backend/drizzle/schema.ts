import { pgTable, index, unique, pgEnum, bigint, timestamp, doublePrecision, text, uuid, boolean, serial, uniqueIndex, integer, jsonb, interval } from "drizzle-orm/pg-core"
...
export const broadcastsSegments = pgTable("broadcasts_segments", {
	id: serial('id').primaryKey().notNull(),createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
...
export const unsubscribedMessages = pgTable("unsubscribed_messages", {
	...
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	replyTo: bigint("reply_to", { mode: "number" }).notNull().references(() => broadcastSentMessageStatus.id, { onDelete: "cascade" } ),
});
