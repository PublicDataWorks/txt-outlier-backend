...
export const broadcastMessagesSentStatus = pgTable('broadcast_messages_sent_status', {
  ...
  missiveConversationId: uuid('missive_conversation_id').notNull().references(() => conversations.id, {
    onUpdate: 'cascade',
  }),
}, (table) => {
  return {
    broadcastSentMessagesMissiveMessageIdKey: unique('broadcast_sent_messages_missive_message_id_key').on(
      table.missiveMessageId,
    ),
  }
})
