...
const buildDigestBlocks = (data: {
  ...
  if (data.tagCounts.length > 0) {
    const tagLines = data.tagCounts
      ...
      .map((row) => {
        ...
        return `• *${tag}* — ${row.count}${
          deltaEmoji(row.count, data.priorTagCounts.get(row.tag) ?? 0)
        }`;
      })
      .join("\n");
    blocks.push({
  ...
  if (data.unmetDemandExamples.length > 0) {
    const exampleLines = data.unmetDemandExamples
      .map((example) => {
        const link = example.webUrl
          ? ` <${example.webUrl}|Open in Missive>`
          : "";
      ...
      [
        ...
        },
        {
          ...
          text: {
            type: "mrkdwn",
            text:
              "Quiet week — no conversations were analyzed in the last 7 days.",
          },
        },
      ],
