...
describe(
  ...
  () => {
    it("should successfully send broadcast now", async () => {
      ...
      assertEquals(
        Math.abs(
          new Date(updatedBroadcast.runAt).getTime() - new Date().getTime(),
        ) < 5000,
        true,
    ...
    it("should fail when broadcast is scheduled within 90 minutes", async () => {
      ...
      const { message: actualErrorMessage }: { message: string } = await error
        .context.json();
      assertEquals(
        actualErrorMessage,
    ...
    it("should successfully send broadcast when runAt is null", async () => {
      ...
      assertEquals(
        updatedBroadcast.runAt instanceof Date ||
          typeof updatedBroadcast.runAt === "string",
        true,
