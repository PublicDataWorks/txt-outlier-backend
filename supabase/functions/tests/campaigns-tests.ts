...
describe(
  ...
  () => {
    it("should create a new campaign with required fields", async () => {
      ...
      assertEquals(
        Math.floor(newCampaign.runAt.getTime() / 1000),
        futureTimestamp,
      );
      ...
      // @ts-ignore - Segments is stored as JSONB, TypeScript doesn't know its structure
      assertEquals(newCampaign.segments.excluded, [{ id: label2.id }, {
        id: label3.id,
      }]);
    });
    ...
    it("should handle null, empty, and whitespace campaignLabelName", async () => {
      ...
      const { data: nullLabelData } = await client.functions.invoke(
        FUNCTION_NAME,
        {
          method: "POST",
          body: {
      ...
      const { data: emptyLabelData } = await client.functions.invoke(
        FUNCTION_NAME,
        {
          method: "POST",
          body: {
      ...
      const { data: whitespaceLabelData } = await client.functions.invoke(
        FUNCTION_NAME,
        {
          method: "POST",
          body: {
      ...
      assertEquals(
        whitespaceLabelData.title,
        "Campaign with whitespace-only label",
      );
    ...
    it("should reject campaignLabelName containing forward slash", async () => {
      ...
      assertEquals(
        errorData.message.includes(
          "Campaign label name cannot contain forward slash",
        ),
        true,
      );
    ...
    it("should create a campaign with all fields", async () => {
      ...
      assertEquals(
        Math.floor(savedCampaign.runAt.getTime() / 1000),
        futureTimestamp,
      );
      // @ts-ignore - Segments is stored as JSONB, TypeScript doesn't know its structure
    ...
    it("should return 400 when runAt is not a valid timestamp", async () => {
      ...
      const errorData = await error.context.json();
      assertEquals(
        errorData.message.includes("[runAt] Expected number, received string"),
        true,
      );
    });
    ...
    it("should return 400 when runAt is in the past", async () => {
      ...
      const errorData = await error.context.json();
      assertEquals(
        errorData.message.includes("Run time must be in the future"),
        true,
      );
    });
    ...
    it("should return 400 when delay is less than or equal to 0", async () => {
      ...
      const negativeDelayResponse = await client.functions.invoke(
        FUNCTION_NAME,
        {
          method: "POST",
          body: {
      ...
      const negativeDelayError = await negativeDelayResponse.error.context
        .json();
...
describe("PATCH", { sanitizeOps: false, sanitizeResources: false }, () => {
  it("should update an upcoming campaign", async () => {
    ...
    assertEquals(
      Math.floor(updatedCampaign.runAt.getTime() / 1000),
      newFutureTimestamp,
    );
  });
  ...
  it("should return 400 when no fields provided", async () => {
    ...
    const errorData = await error.context.json();
    assertEquals(
      errorData.message.includes("At least one field must be provided"),
      true,
    );
  });
  ...
  it("should not allow updating campaignLabelName field", async () => {
    ...
    const nullResponse = await client.functions.invoke(
      `${FUNCTION_NAME}${campaign.id}/`,
      {
        method: "PATCH",
        body: {
  ...
  it("should convert file-based campaign to segment-based", async () => {
    ...
    const file = new File([csvContent], "file-campaign.csv", {
      type: "text/csv",
    });
    ...
    const updateResponse = await client.functions.invoke(
      `${FUNCTION_NAME}${campaignId}/`,
      {
        method: "PATCH",
        body: {
...
describe("GET", { sanitizeOps: false, sanitizeResources: false }, () => {
  ...
  it("should convert string statistics to numbers for past campaigns", async () => {
    ...
    const author1 = await createAuthor("+12345678901", {
      unsubscribed: false,
      exclude: false,
    });
    const author2 = await createAuthor("+12345678902", {
      unsubscribed: false,
      exclude: false,
    });
...
describe("GET /campaigns/segments/", {
  sanitizeOps: false,
  ...
  it("should return segments with id and name", async () => {
    ...
    // Create authors
    const author1 = await createAuthor("+1234567890", {
      unsubscribed: false,
      exclude: false,
    });
    const author2 = await createAuthor("+1234567891", {
      unsubscribed: false,
      exclude: false,
    });
    const author3 = await createAuthor("+1234567892", {
      unsubscribed: true,
      exclude: false,
    });
    const author4 = await createAuthor("+1234567893", {
      unsubscribed: false,
      exclude: true,
    });
  ...
  it("should handle labels with only archived conversations", async () => {
    ...
    const author = await createAuthor("+1234567890", {
      unsubscribed: false,
      exclude: false,
    });
    const conversation = await createConversation();
  ...
  it("should handle labels with only unsubscribed or excluded authors", async () => {
    ...
    const unsubscribedAuthor = await createAuthor("+1234567890", {
      unsubscribed: true,
      exclude: false,
    });
    const excludedAuthor = await createAuthor("+1234567891", {
      unsubscribed: false,
      exclude: true,
    });
...
describe("POST /campaigns/recipient-count/", {
  sanitizeOps: false,
  sanitizeResources: false,
}, () => {
  it("should return correct recipient count for given segments", async () => {
    ...
    const author1 = await createAuthor("+1111111111", {
      unsubscribed: false,
      exclude: false,
    });
    const author2 = await createAuthor("+2222222222", {
      unsubscribed: false,
      exclude: false,
    });
    const author3 = await createAuthor("+3333333333", {
      unsubscribed: false,
      exclude: false,
    });
    const author4 = await createAuthor("+4444444444", {
      unsubscribed: false,
      exclude: false,
    });
    const author5 = await createAuthor("+5555555555", {
      unsubscribed: true,
      exclude: false,
    }); // unsubscribed
    ...
    const singleSegmentResponse = await client.functions.invoke(
      `${FUNCTION_NAME}recipient-count/`,
      {
        method: "POST",
        body: {
          segments: {
    ...
    const multipleIncludedResponse = await client.functions.invoke(
      `${FUNCTION_NAME}recipient-count/`,
      {
        method: "POST",
        body: {
    ...
    const excludedSegmentResponse = await client.functions.invoke(
      `${FUNCTION_NAME}recipient-count/`,
      {
        method: "POST",
        body: {
    ...
    const unsubscribedSegmentResponse = await client.functions.invoke(
      `${FUNCTION_NAME}recipient-count/`,
      {
        method: "POST",
        body: {
  ...
  it("should return 400 for invalid segment format", async () => {
    const invalidResponse = await client.functions.invoke(
      `${FUNCTION_NAME}recipient-count/`,
      {
        method: "POST",
        body: {
  ...
  it("should return 400 for missing segments", async () => {
    const missingSegmentsResponse = await client.functions.invoke(
      `${FUNCTION_NAME}recipient-count/`,
      {
        method: "POST",
        body: {},
  ...
  it("should count correctly with AND groups", async () => {
    ...
    const author1 = await createAuthor("+6666666666", {
      unsubscribed: false,
      exclude: false,
    });
    const author2 = await createAuthor("+7777777777", {
      unsubscribed: false,
      exclude: false,
    });
    const author3 = await createAuthor("+8888888888", {
      unsubscribed: false,
      exclude: false,
    });
    ...
    const andGroupResponse = await client.functions.invoke(
      `${FUNCTION_NAME}recipient-count/`,
      {
        method: "POST",
        body: {
  ...
  it("should reject null excluded segments", async () => {
    ...
    const nullExcludedResponse = await client.functions.invoke(
      `${FUNCTION_NAME}recipient-count/`,
      {
        method: "POST",
        body: {
...
describe("File-based POST/PATCH", {
  sanitizeOps: false,
  sanitizeResources: false,
}, () => {
  it("should create a campaign from file upload", async () => {
    ...
    const csvContent = phoneNumbers.join("\n");
    const file = new File([csvContent], "test-numbers.csv", {
      type: "text/csv",
    });
  ...
  it("should create a file-based campaign with existing campaignLabelName", async () => {
    ...
    const csvContent = phoneNumbers.join("\n");
    const file = new File([csvContent], "test-label-numbers.csv", {
      type: "text/csv",
    });
    ...
    formData.append(
      "firstMessage",
      "Test message for file campaign with label",
    );
    formData.append("runAt", futureTimestamp.toString());
  ...
  it("should return 400 when file is missing", async () => {
    ...
    assertEquals(
      errorData.message,
      "File is required for file-based campaigns",
    );
  });
  ...
  it("should return 400 for invalid file format", async () => {
    ...
    assertEquals(
      errorData.message,
      "Please upload a CSV file with phone numbers",
    );
  });
...
describe("DELETE", { sanitizeOps: false, sanitizeResources: false }, () => {
  ...
  it("should delete a file-based campaign and its file", async () => {
    ...
    const phoneNumbers = ["+11234567890", "+19876543210"];
    const csvContent = phoneNumbers.join("\n");
    const file = new File([csvContent], "delete-test.csv", {
      type: "text/csv",
    });
  ...
  it("should return 400 for non-existent campaign", async () => {
    ...
    assertEquals(
      errorData.message,
      "Campaign not found or is not an upcoming campaign",
    );
  });
  ...
  it("should return 400 for processed campaigns", async () => {
    ...
    const { error } = await client.functions.invoke(
      `${FUNCTION_NAME}${processedCampaign.id}/`,
      {
        method: "DELETE",
      },
    );
