...
}/functions/v1/slack-interactions`;
...
const fetchAnalysis = async (id: number) => {
  const [row] = await supabase.select().from(conversationAnalyses).where(
...
describe(
  ...
  () => {
    it("rejects a request with an invalid signature", async () => {
      ...
      const payload = blockActionsPayload(analysis.id, {
        id: "U1",
        name: "jdoe",
      });

      const response = await postInteraction(payload, {
        signatureOverride: "v0=0000000000000000",
      });
    ...
    it("is idempotent: a second click by a different user does not change who promoted it", async () => {
      ...
      await postInteraction(
        blockActionsPayload(analysis.id, { id: "U1", name: "Jane Doe" }),
      );
      const afterFirst = await fetchAnalysis(analysis.id);

      await postInteraction(
        blockActionsPayload(analysis.id, { id: "U2", name: "Bob Smith" }),
      );
