...
describe("buildAnalysisMessageBlocks", {
  sanitizeOps: false,
  ...
  it("masks the phone number to its last 4 digits and includes message count/date/link in context", () => {
    ...
    assertEquals(blocks[3], {
      ...
      elements: [{
        type: "mrkdwn",
        text:
          "...1234  •  5 messages  •  2026-07-10  •  <https://mail.missiveapp.com/#inbox/conversations/a1a1a1a1|" +
          "Open in Missive>",
      }],
    });
  });
  ...
  it('uses singular "message" wording when there is exactly one message', () => {
    const blocks = buildAnalysisMessageBlocks(baseAnalysis, {
      ...baseConversation,
      messageCount: 1,
    });
  ...
  it("inserts an unmet demand warning block between context and actions when unmetDemand is true", () => {
    const analysis = {
      ...baseAnalysis,
      unmetDemand: true,
      unmetDemandReason: "Needed emergency shelter referral",
    };
    ...
    assertEquals(blocks[4], {
      ...
      text: {
        type: "mrkdwn",
        text: ":warning: *Unmet demand:* Needed emergency shelter referral",
      },
    });
  ...
  it('falls back to "Not specified" when unmetDemand is true but no reason is given', () => {
    ...
    assertEquals(
      blocks[4].text.text,
      ":warning: *Unmet demand:* Not specified",
    );
  });
