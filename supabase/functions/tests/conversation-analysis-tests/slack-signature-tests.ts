...
describe('verifySlackSignature', { sanitizeOps: false, sanitizeResources: false }, () => {
  ...
  it('accepts a correctly signed, fresh request', async () => {
    ...
    assert(await verifySlackSignature(SIGNING_SECRET, timestamp, signature, body))
  })
  ...
  it('rejects a stale timestamp outside the 5-minute window', async () => {
    ...
    const signature = await sign(SIGNING_SECRET, timestamp, body)
    const result = await verifySlackSignature(SIGNING_SECRET, timestamp, signature, body)
  ...
  it('rejects a tampered signature', async () => {
    ...
    const result = await verifySlackSignature(SIGNING_SECRET, timestamp, tampered, body)
  ...
  it('rejects a signature computed over a different body', async () => {
    const timestamp = Math.floor(Date.now() / 1000).toString()
    const signature = await sign(SIGNING_SECRET, timestamp, body)
    const result = await verifySlackSignature(SIGNING_SECRET, timestamp, signature, body + 'tampered')
  ...
  it('rejects when the timestamp header is missing', async () => {
    ...
    const result = await verifySlackSignature(SIGNING_SECRET, '', signature, body)
  ...
  it('rejects when the signature header is missing', async () => {
    const timestamp = Math.floor(Date.now() / 1000).toString()
    const result = await verifySlackSignature(SIGNING_SECRET, timestamp, '', body)

    assertEquals(result, false)
  })

  it('rejects a non-numeric timestamp header', async () => {
    const result = await verifySlackSignature(SIGNING_SECRET, 'not-a-number', 'v0=whatever', body)
  ...
  it('rejects when signature and computed digest have different lengths', async () => {
    const timestamp = Math.floor(Date.now() / 1000).toString()
    const result = await verifySlackSignature(SIGNING_SECRET, timestamp, 'v0=short', body)
  ...
  it('correctly compares equal-length hex digests that differ only in a middle byte', async () => {
    ...
    const tampered = signature.slice(0, middle) + flippedChar + signature.slice(middle + 1)
