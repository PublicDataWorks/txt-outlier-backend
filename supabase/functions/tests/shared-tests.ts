...
describe("DateUtils", () => {
  describe("advance", () => {
    it("should advance time by milliseconds", () => {
      ...
      assertEquals(
        diff >= 1000 && diff < 1100,
        true,
        "Should advance approximately 1 second",
      );
    });
  });
  ...
  describe("calculateNextScheduledTime", {
    ...
    it("should find next time on same day", async () => {
      ...
      const laterDetroitDateTime = formatInTimeZone(
        twoHoursLater,
        DETROIT_TIMEZONE,
      ...
      const resultDetroitTime = formatInTimeZone(
        resultInDetroit,
        DETROIT_TIMEZONE,
        "HH:mm:ss",
      );
      ...
      const resultDay = formatInTimeZone(
        resultInDetroit,
        DETROIT_TIMEZONE,
        "eee",
      ).toLowerCase();
    ...
    it("should find next time on next day when all times today have passed", async () => {
      ...
      const nowInDetroit = formatInTimeZone(
        now,
        DETROIT_TIMEZONE,
        "yyyy-MM-dd HH:mm:ss eee",
      );
      const tomorrowInDetroit = formatInTimeZone(
        tomorrow,
        DETROIT_TIMEZONE,
        "yyyy-MM-dd HH:mm:ss eee",
      );
    ...
    it("should wrap around to next week when no remaining times this week", async () => {
      ...
      const resultOffset = formatInTimeZone(
        resultInDetroit,
        DETROIT_TIMEZONE,
        "xxx",
      );
      ...
      if (isDSTChange) {
        ...
        const [originalHour, originalMinSec] = [
          originalTime.substring(0, 2),
          originalTime.substring(2),
        ];
        const [resultHour, resultMinSec] = [
          resultTime.substring(0, 2),
          resultTime.substring(2),
        ];

        assertEquals(
          resultMinSec,
          originalMinSec,
          "Minutes and seconds should match",
        );
        assert(
      ...
      const nowDate = new Date(
        formatInTimeZone(now, DETROIT_TIMEZONE, "yyyy-MM-dd"),
      );
      const resultDate = new Date(
        formatInTimeZone(resultInDetroit, DETROIT_TIMEZONE, "yyyy-MM-dd"),
      );
      const diffInDays = Math.round(
        (resultDate.getTime() - nowDate.getTime()) / (24 * 60 * 60 * 1000),
      );
