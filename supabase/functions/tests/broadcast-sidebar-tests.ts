...
describe(
  ...
  () => {
    it("should return default pagination (5 past + 1 upcoming)", async () => {
      ...
      assertEquals(
        data.upcoming.secondMessage,
        "Future broadcast second message",
      );
      ...
      assertEquals(
        data.past[0].secondMessage,
        "Past broadcast 1 second message",
      );

      assertEquals(data.past[1].firstMessage, "Past broadcast 2");
      assertEquals(
        data.past[1].secondMessage,
        "Past broadcast 2 second message",
      );
    });
    ...
    it("should set runAt from broadcast_settings for upcoming broadcast", async () => {
      ...
      const tomorrowDay = detroitTomorrow.toLocaleDateString("en-US", {
        weekday: "short",
      }).toLowerCase();
    ...
    it("should correctly count messages with different statuses", async () => {
      ...
      const broadcastData = data.past.find((b: Broadcast) =>
        b.id === broadcast.id
      );
