...
export const createBroadcastMessageStatus = async ({
  ...
  const sentMessage = {
    ...
    broadcastId: id,
    isSecond,
    twilioSentStatus: twilioSentStatus ||
      faker.random.arrayElement(["delivered", "sent", "undelivered"]),
    message: isSecond
      ? `Second message ${faker.lorem.sentence()}`
      : `First message ${faker.lorem.sentence()}`,
