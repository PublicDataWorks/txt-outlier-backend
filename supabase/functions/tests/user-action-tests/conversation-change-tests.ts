...
describe(
  ...
  () => {
    it("closed", async () => {
      ...
      const existingConvoUsers = await supabase.select().from(
        conversationsUsers,
      );
    ...
    it("assignee change with labels - should upsert labels", async () => {
      ...
      const parentLabel = createdLabels.find((l) =>
        l.id === "13405e0f-8551-4ea7-a303-7552aee0bce4"
      )!;
      ...
      const nestedLabel = createdLabels.find((l) =>
        l.id === "37113989-97e3-4121-a903-f85af6bfaaa0"
      )!;
      ...
      const conversationLabels = await supabase.select().from(
        conversationsLabels,
      );
      ...
      assertEquals(
        conversationLabels.every((cl) => cl.isArchived === false),
        true,
      );
    });
    ...
    it("assignee change with label removal - should archive removed labels", async () => {
      ...
      const requestWithOneLabel = JSON.parse(
        JSON.stringify(conversationAssigneeChangeRequest),
      );
      requestWithOneLabel.conversation.shared_labels = [
        requestWithOneLabel.conversation.shared_labels[0],
      ];
      ...
      const parentLabelLink = updatedLabels.find((cl) =>
        cl.labelId === "13405e0f-8551-4ea7-a303-7552aee0bce4"
      )!;
      const nestedLabelLink = updatedLabels.find((cl) =>
        cl.labelId === "37113989-97e3-4121-a903-f85af6bfaaa0"
      )!;
    ...
    it("assignee change with all labels removed - should archive all labels", async () => {
      ...
      const requestWithNoLabels = JSON.parse(
        JSON.stringify(conversationAssigneeChangeRequest),
      );
