...
describe(
  ...
  () => {
    it("receive reply after sending first message", async () => {
      const queue_message = await supabase.execute(
        pgmqSend(SECOND_MESSAGES_QUEUE_NAME, '{"hello": "world"}', 10),
      );
      ...
      assertEquals(
        ...
        0,
        "TwilioMessages should be empty initially",
      );
...
describe(
  ...
  () => {
    it("unsubscribe after receiving broadcast", async () => {
      ...
      const unsubscribeBefore = await supabase.select().from(
        unsubscribedMessages,
      );
      ...
      const unsubscribeAfter = await supabase.select().from(
        unsubscribedMessages,
      );
      ...
      assertEquals(
        unsubscribeAfter[0].twilioMessageId,
        unsubscribeMsg.message!.id,
      );
    });
    ...
    it("stop message should NOT be marked as reply", async () => {
      ...
      assertEquals(incomingMessages.length, 1);
      assertEquals(
        incomingMessages[0].isReply,
        false,
        "STOP messages should not be marked as replies",
      );
      assertEquals(
        incomingMessages[0].replyToBroadcast,
        null,
        "STOP messages should not have replyToBroadcast set",
      );
      assertEquals(
        incomingMessages[0].replyToCampaign,
        null,
        "STOP messages should not have replyToCampaign set",
      );
    });
