...
describe("DubLinkShortener", () => {
  ...
  describe("shortenLinksInMessage", () => {
    it("should return the message unchanged when there are no URLs", async () => {
      ...
      const [result, changed] = await DubLinkShortener.shortenLinksInMessage(
        message,
        broadcastId,
      );
    ...
    it("should shorten URLs using the configured tag name", async () => {
      ...
      assertEquals(
        result,
        "Check out https://dub.sh/abc123 for more information",
      );
    ...
    it("should use existing shortened links if available", async () => {
      ...
      assertEquals(
        result,
        "Check out https://dub.sh/abc123 and https://dub.sh/def456",
      );
      assertEquals(changed, true);
    ...
    it("should handle multiple occurrences of the same URL", async () => {
      // Setup
      const message =
        "Check https://example.com here and https://example.com there";
      const broadcastId = 123;
      const tagName = "txt-messages";
      ...
      assertEquals(
        result,
        "Check https://dub.sh/abc123 here and https://dub.sh/abc123 there",
      );
      assertEquals(changed, true);
    ...
    it("should handle GitHub repository URLs correctly", async () => {
      // Setup
      const message =
        "https://dub.sh/GhVrND3 https://github.com/PublicDataWorks/txt-outlier-backend/pull/85";
      const broadcastId = 123;
      const tagName = "txt-messages";
    ...
    it("should properly handle URLs with trailing punctuation", async () => {
      // Setup
      const message =
        "Check out https://example.com. Also visit https://example.org, and maybe https://example.net!";
      const broadcastId = 123;
      const tagName = "txt-messages";
    ...
    it("should correctly identify and handle URLs with parentheses and other special punctuation", async () => {
      // Setup
      const message =
        "See this page (https://example.com) and this too: https://example.org;";
      const broadcastId = 123;
      const tagName = "txt-messages";
    ...
    it("should shorten valid URLs while filtering out malformed ones", async () => {
      ...
      assertEquals(
        result,
        "Check https://dub.sh/abc123 and https://[invalid here",
      );
      assertEquals(changed, true);
      sinon.assert.calledWith(dubMock.links.createMany, [{
        url: "https://example.com",
    ...
    it("should handle mix of valid, excluded, and malformed URLs", async () => {
      const message =
        "Check https://example.com and https://bit.ly/abc and https://[invalid here";
      const broadcastId = 123;
      const tagName = "txt-messages";
      ...
      assertEquals(
        result,
        "Check https://dub.sh/abc123 and https://bit.ly/abc and https://[invalid here",
      );
      assertEquals(changed, true);
      sinon.assert.calledWith(dubMock.links.createMany, [{
        url: "https://example.com",
    ...
    it("should return original message when DUB_TAG_NAME is not set", async () => {
      ...
      sinon.assert.calledWith(
        console.error as sinon.SinonStub,
        "DUB_TAG_NAME environment variable is not set",
      );
    });
  });
});
