  ...
  () => {
    it("should successfully make a broadcast with segments", async () => {
      ...
      await client.functions.invoke(FUNCTION_NAME, {
        method: "POST",
        body: {
          run_at_utc: new Date().toISOString().slice(0, 16).replace("T", " "),
        },
      });
      ...
      assertEquals(
        updatedBroadcast.editable,
        false,
        "Original broadcast should not be editable",
      );
      ...
      assertEquals(
        newBroadcast.firstMessage,
        "Test first message",
        "First message should match",
      );
      assertEquals(
        newBroadcast.secondMessage,
        "Test second message",
      ...
      assertEquals(
        newBroadcast.editable,
        true,
        "New broadcast should be editable",
      );
      assertEquals(
        newBroadcast.runAt,
        null,
        "New broadcast should not have runAt set",
      );

      assertEquals(
        newBroadcast.broadcastToSegments.length,
        1,
        "Should have one segment",
      );
      assertEquals(
        newBroadcast.broadcastToSegments[0].segment.name,
        "make-test",
        "Segment name should be Test",
      );
      assertEquals(
        newBroadcast.broadcastToSegments[0].ratio,
        10,
        "Segment ratio should be 100",
      );
    ...
    it("should queue messages correctly", async () => {
      ...
      assertEquals(
        firstMessage.second_message,
        "Test second message",
        "Second message should match",
      );
      assertEquals(firstMessage.delay, 600, "Delay should match");
      assertEquals(
        typeof firstMessage.recipient_phone_number,
        "string",
        "Should have phone number",
      );
      assertEquals(
        typeof firstMessage.segment_id,
        "number",
        "Should have segment ID",
      );
      ...
      queuedMessages.forEach((qm: { message: any }, index: number) => {
        ...
        assertEquals(
          ...
          broadcast.id,
          `Message ${index} broadcast ID should match`,
        );
        assertEquals(
          message.first_message,
          "Test first message",
          `Message ${index} first message should match`,
        );
        assertEquals(
          message.second_message,
          "Test second message",
          `Message ${index} second message should match`,
        );
        assertEquals(message.delay, 600, `Message ${index} delay should match`);
        assertEquals(
          typeof message.recipient_phone_number,
          "string",
          `Message ${index} should have phone number`,
        );
        assertEquals(
          typeof message.segment_id,
          "number",
          `Message ${index} should have segment ID`,
        );
      });

      const totalQueuedMessages = await supabase.execute(
        sql.raw(
          "SELECT COUNT(*) as count FROM pgmq.q_broadcast_first_messages",
        ),
      );
      assertEquals(
        totalQueuedMessages[0].count,
        "2",
        "Should have messages for all authors",
      );
    });
    ...
    it("should fail when run_at_utc does not match current time", async () => {
      ...
      const future_run_at_utc = futureDate.toISOString().slice(0, 16).replace(
        "T",
        " ",
      );
      ...
      assertEquals(
        updatedBroadcast.editable,
        true,
        "Broadcast should remain editable",
      );
      ...
      assertEquals(
        newBroadcast,
        undefined,
        "No new broadcast should be created",
      );
    });
