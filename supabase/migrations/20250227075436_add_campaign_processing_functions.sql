...
BEGIN
    ...
    WITH RECURSIVE
      ...
      matching_phones AS (
        SELECT DISTINCT cl.author_phone_number, fs.item
        FROM all_segments fs
        JOIN LATERAL (
          -- For each segment, get matching phone numbers
          SELECT cl.author_phone_number
          FROM conversations_labels cl
          JOIN eligible_authors ea ON cl.author_phone_number = ea.phone_number
          WHERE
            cl.is_archived = FALSE
            AND cl.author_phone_number IS NOT NULL
...
SELECT cron.schedule(
  'check_and_run_campaigns',
  '* * * * *',
  'SELECT check_and_run_campaigns();'
);
----------------------------------------------------------------
